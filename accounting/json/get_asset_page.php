...
if (isset($_POST['item_id']) && !empty($_POST['item_id'])) {
  ...
  if ($count > 0) {
    while ($row = $stmt->fetchAll(PDO::FETCH_ASSOC)) { // Loop for queries
      ...
      $row[0]['init_quantity'] = $row[0]['init_price'] / $row[0]['amount'];
      $row[0]['vat'] = number_format($row[0]['vat_amount'], 2, '.', ',');
      $row[0]['with'] = number_format($row[0]['witholding'], 2, '.', ',');
      $row[0]['amount'] = number_format($row[0]['amount'], 2, '.', ',');
      $row[0]['total'] = number_format($row[0]['total'], 2, '.', ',');
      $row[0]['init_price'] = number_format($row[0]['init_price'], 2, '.', ',');
      $row[0]['carry'] = number_format($carry, 2, '.', ',');
      $result = $row[0];
    }
  } else {
  }
