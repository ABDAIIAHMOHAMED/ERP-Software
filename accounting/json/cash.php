...
$stmt = $con->prepare("SELECT * FROM `manufacturer_transactions` WHERE credit = '0' AND `payment_method` = 'cash'");
$stmt->execute();
$count = $stmt->rowCount();
if ($count > 0) {
  while ($row = $stmt->fetchAll(PDO::FETCH_ASSOC)) { // Loop for queries
...
if ($count > 0) {
  while ($row = $stmt->fetchAll(PDO::FETCH_ASSOC)) { // Loop for queries
    foreach ($row as $supp) {
      $statement = '<a href="vtax.php" target="_blank">Payment to VAT Tax.</a> <span class="sign out">-</span> ';
      $date = $supp['date'];
      $credit = number_format(0, 2);
      $debit = number_format($supp['debit'], 2);
      $cash = array(
...
if ($count > 0) {
  while ($row = $stmt->fetchAll(PDO::FETCH_ASSOC)) { // Loop for queries
    foreach ($row as $supp) {
      $id = $supp['item_id'];
      $stmt2 = $con->prepare("SELECT * FROM `assets` WHERE `id` = '$id'");
      $stmt2->execute();
      $row2 = $stmt2->fetch(PDO::FETCH_ASSOC);
      $name = $row2['name'];
...
if ($count > 0) {
  while ($row = $stmt->fetchAll(PDO::FETCH_ASSOC)) { // Loop for queries
    foreach ($row as $supp) {
      ...
      $statement = 'Transfered money trom <a href="bankdetails.php?uid=' . $from_bank_id . '" target="_blank">' . $from_bank_number  . ' (' . $from_bank_name . ')</a> <span class="sign in">+</span> ';
      $date = $supp['date'];
      $debit = number_format(0, 2);
...
if ($count > 0) {
  while ($row = $stmt->fetchAll(PDO::FETCH_ASSOC)) { // Loop for queries
    foreach ($row as $supp) {
      ...
      $row2 = $stmt2->fetch(PDO::FETCH_ASSOC);
      $from_bank_id = $row2['id'];
      $statement = 'Transfered money to <a href="bankdetails.php?uid=' . $from_bank_id . '" target="_blank">' . $from_bank_number  . ' (' . $from_bank_name . ')</a> <span class="sign out">-</span> ';
      $date = $supp['date'];
