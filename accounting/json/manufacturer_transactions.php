...
if (isset($_GET['uid']) && !empty($_GET['uid'])) {
  ...
  $id = $_GET['uid'];
  $transactions = array();
  $stmt = $con->prepare("SELECT DISTINCT `order_id`,`balance_due`,`status`,`type`,`order_total`,`paid`,`due_to`,`created_time`,`received` FROM `manufacturer_orders` WHERE `supplier_id` = '$id'");
  $stmt->execute();
  $count = $stmt->rowCount();
  $total_due = 0;
  if ($count > 0) {
