...
if (login_redirect() == 1) {
  if (check_id('bank_account')) {
    if (check_user_role(1) == 1 || check_user_role(3000) == 1 || check_user_role(3003) == 1) {
      $path = (@$_SERVER["HTTPS"] == "on") ? "https://" : "http://";
      $path .= $_SERVER["SERVER_NAME"] . dirname($_SERVER["PHP_SELF"]);
      $user_info = login_info();
      $info = info('bank_account');
      if (check_user_role(1) == 1 || check_user_role(3000) == 1 || check_user_role(3005) == 1) {
        import_transfer_money($info['bank_name'], $info['account_number']);
      }
?>
      <!-- #F1DDCA -->
      <!-- #AD6420-->
      <div class="att-part vat-part">
        <div class="title-head">
          <input type="hidden" id="account_number_id" value="<?= $info['account_number'] ?>">
          <input type="hidden" id="bank_name_id" value="<?= $info['bank_name'] ?>">
          <div class="headso">
            <h3>Bank Account Statement
              <div class="inputs">
            ...
            <hr class="head-line">
            <p class="navigator"><a href="<?= $path ?>">Dashboard</a> / <a href="<?= $path . "/banks.php" ?>">Bank Accounts</a> / Bank Account Statement</p>
          </div>
            ...
            <script>
              $(function() {
                ...
                $("#ExportReporttoPdf").on("click", function() {
                  ...
                  printWindow.document.write('</head><body><div class="container"><div class="header_container"><div class="lt-head"><h3>Bank Account for ' + $("#bank_name_id").val() + ' ( ' + $("#account_number_id").val() + ' )</h3><span class="dt-range">From <span class="bold">' + $("#create-date-filter").val() + '</span>  To <span class="bold">' + $("#end-date-filter").val() + '</span></span></div><div class="rt-head"><div class="logo_img"><img src="../images/logo.png" alt="Company_logo" /></div></div></div>');
          ...
          <div class="cash-report">
            <div class="widget-bx cash-box w100">
              <div class="widget-txt">
                ...
                <div class="widget-value"><?php if (get_total_bank($info['bank_name'], $info['account_number']) < 0) {
                                            echo "- ";
                                          } ?><span class="w-value" id="total_cash" data-value="<?php if (get_total_bank($info['bank_name'], $info['account_number']) < 0) {
                                                                                                  echo -1 * get_total_bank($info['bank_name'], $info['account_number']);
                                                                                                } else {
                                                                                                  echo get_total_bank($info['bank_name'], $info['account_number']);
            ...
            <div class="widget-bx cash-box w50 cashin" style="animation-delay:0.16s">
              <div class="widget-txt">
                ...
                <div class="widget-value">+ <span class="w-value" id="total_deposits" data-value="<?= get_in_bank($info['bank_name'], $info['account_number']) ?>">0</span> <span class="unit">E£</span></div>
              </div>
            </div>
            <div class="widget-bx cash-box w50 cashout" style="animation-delay:0.32s">
              <div class="widget-txt">
                <div class="widget-head">Withdrawals</div>
                <div class="widget-value">- <span class="w-value" id="total_withdrawals" data-value="<?= get_out_bank($info['bank_name'], $info['account_number']) ?>">0</span> <span class="unit">E£</span></div>
              </div>
            </div>
            <div class="cash-details">
              ...
              <form method="POST" class="cash-box" id="edit_bank">
                <div class="cash-sec">
                  <label class="cash-title" for="bank_name">Bank Name</label>
                  <div class="cash-info">
                    <select name="bank_name" id="bank_name" required value="<?= $info['bank_name'] ?>">
                      <?php
                      $options = array(
                        'البنك الأهلى المصرى' => 'البنك الأهلى المصرى',
                        "بنك الإسكندرية" => "بنك الإسكندرية",
                        "بنك ابو ظبي التجاري – مصر" => "بنك ابو ظبي التجاري – مصر",
                        "مصرف أبو ظبي الإسلامي" => "مصرف أبو ظبي الإسلامي",
                        "البنك الأهلي المتحد - مصر" => "البنك الأهلي المتحد - مصر",
                        "البنك العربي الافريقي" => "البنك العربي الافريقي",
                        "البنك العربي" => "البنك العربي",
                        "المصرف العربي الدولي" => "المصرف العربي الدولي",
                        "بنك التجاري وفا" => "بنك التجاري وفا",
                        "بنك القاهرة" => "بنك القاهرة",
                        "بنك مصر" => "بنك مصر",
                        "البنك المركزي المصري" => "البنك المركزي المصري",
                        "سيتي بنك" => "سيتي بنك",
                        "البنك التجاري الدولي" => "البنك التجاري الدولي",
                        "بنك كريدي أجريكول مصر" => "بنك كريدي أجريكول مصر",
                        "البنك العقاري المصري العربي" => "البنك العقاري المصري العربي",
                        "البنك المصري الخليجي" => "البنك المصري الخليجي",
                        "بنك الإمارات دبي الوطني مصر" => "بنك الإمارات دبي الوطني مصر",
                        "بنك فيصل الاسلامى" => "بنك فيصل الاسلامى",
                        'Export Development Bank of Egypt' => 'Export Development Bank of Egypt',
                        "بنك فيصل الاسلامى" => "بنك فيصل الاسلامى",
                        "HSBC Bank Egypt S.A.E" => "HSBC Bank Egypt S.A.E",
                        "بنك التعمير والإسكان" => "بنك التعمير والإسكان",
                        "بنك المشرق" => "بنك المشرق",
                        "بنك الكويت الوطني" => "بنك الكويت الوطني",
                        "QNB ALAHLI" => "QNB ALAHLI",
                        "Societe Arabe Internationale De Banque" => "Societe Arabe Internationale De Banque",
                        "بنك قناة السويس" => "بنك قناة السويس",
                        "المصرف المتحد" => "المصرف المتحد",
                        "aiBANK" => "aiBANK",
                        "بنك البركة" => "بنك البركة",
                      );

                      foreach ($options as $value => $label) {
                        $selected = ($info['bank_name'] == $value) ? 'selected' : '';
                ...
                <div class="cash-sec">
                  <label class="cash-title" for="account_number">Account Number</label>
                  <div class="cash-info">
                    <input type="text" pattern="[0-9]*" name="account_number" id="account_number" value="<?= $info['account_number'] ?>">
                    <input type="hidden" name="bank_id" value="<?= $info['id'] ?>">
                  </div>
                </div>
      ...
      <script>
        $(document).ready(function() {
          ...
          $('#edit_bank').submit(function(event) {
            ...
            $.ajax({
              type: 'POST',
              url: 'functions/core/edit-bank.php',
              data: $(this).serialize(),
...
<?php
      ...
      $path = (@$_SERVER["HTTPS"] == "on") ? "https://" : "http://";
      $path .= $_SERVER["SERVER_NAME"] . dirname($_SERVER["PHP_SELF"]);
      header("location:" . $path . "/banks.php");
      exit();
    }
  } else {
    $path = (@$_SERVER["HTTPS"] == "on") ? "https://" : "http://";
