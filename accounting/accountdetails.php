<?php
$pg_name = "Wonder - Share Capital";
session_start();
include "includes/header.inc";
if (login_redirect() == 1) {
  $user_info = login_info();
  if (check_id('share_account')) {
    if (check_user_role(1) == 1 || check_user_role(4000) == 1 || check_user_role(4003) == 1) {
      $info = info('share_account');
      import_share();
      if (check_user_role(1) == 1 || check_user_role(4000) == 1 || check_user_role(4005) == 1) {
        delete_share();
      }
?>
      <div class="att-part vat-part">
        <div class="title-head">
          <div class="headso">
            <input type="hidden" id="account_name" value="<?= $info['name'] ?>">
            <h3><?= $info['name'] ?>'s Account Statement
              <div class="inputs">
            ...
            <hr class="head-line">
            <p class="navigator"><a href="<?= $path ?>">Dashboard</a> / <a href="<?= $path . "capital.php" ?>">Share Capital</a> / <?= $info['name'] ?>'s Account Statement</p>
          </div>
          ...
          <div class="btns">
            ...
            <?php if (check_user_role(1) == 1 || check_user_role(4000) == 1 || check_user_role(4004) == 1) {
            ?>
          ...
          <div class="cash-report">
            <div class="widget-bx cash-box w100">
              <div class="widget-txt">
                ...
                <div class="widget-value"><?php if (get_total_share($info['id']) < 0) {
                                            echo "- ";
                                          } ?><span class="w-value" id="total_cash" data-value="<?php if (get_total_share($info['id']) < 0) {
                                                                                                  echo -1 * get_total_share($info['id']);
                                                                                                } else {
                                                                                                  echo get_total_share($info['id']);
            ...
            <div class="widget-bx cash-box w50 cashin" style="animation-delay:0.16s">
              <div class="widget-txt">
                <div class="widget-head">Funding</div>
                <div class="widget-value">+ <span class="w-value" id="total_deposits" data-value="<?= get_in_share($info['id']) ?>">0</span> <span class="unit">E£</span></div>
              </div>
            </div>
            <div class="widget-bx cash-box w50 cashout" style="animation-delay:0.32s">
              <div class="widget-txt">
                <div class="widget-head">Withdrawals</div>
                <div class="widget-value">- <span class="w-value" id="total_withdrawals" data-value="<?= get_out_share($info['id']) ?>">0</span> <span class="unit">E£</span></div>
              </div>
            </div>
            <div class="cash-details">
              ...
              <form method="POST" class="cash-box" id="edit_bank">
                <div class="cash-sec">
                  <label class="cash-title" for="name">Account Name</label>
                  <div class="cash-info">
        ...
        <?php if (check_user_role(1) == 1 || check_user_role(4000) == 1 || check_user_role(4005) == 1) {
        ?>
          <div class="confirm_delete_page">
            <div class="custome-head">
              <h3>Are you sure to remove a transaction</h3>
              <button class="close_confirm"><i class="fa-solid fa-xmark"></i></button>
            </div>
            <form method="POST">
              <input type="hidden" class="account_id" value="<?= $info['id'] ?>" name="account_id">
          ...
          <form class="" action="" method="POST" enctype="multipart/form-data">
            ...
            <div class="custom-form custom-item-form payment_form">
              <div class="inputs w100 radio_type">
                <label for="fund"><input type="radio" id="fund" name="type" value="1" class="fund" required><span class="radio_label fund">Fund +</span></label>
                <label for="withdraw"><input type="radio" id="withdraw" name="type" value="2" required><span class="radio_label withdraw">Withdraw -</span></label>
              </div>
...
<?php
      ...
      header("location:" . $path . "/capital.php");
      exit();
    }
  } else {
    $path = (@$_SERVER["HTTPS"] == "on") ? "https://" : "http://";
