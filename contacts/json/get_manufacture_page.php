...
if (isset($_POST['order_id']) && !empty($_POST['order_id'])) {
  ...
  if ($count > 0) {
    while ($row = $stmt->fetchAll(PDO::FETCH_ASSOC)) { // Loop for queries
      $stmt2 = $con->prepare("SELECT * FROM `manufacturers` WHERE `id` = '" . $row[0]['supplier_id'] . "'");
      $stmt2->execute();
      ...
      if ($row[0]['storable'] == "1") {
        $stmt3 = $con->prepare("SELECT * FROM `warehouses` WHERE `id` = '" . $row[0]['warehouse_id'] . "'");
        $stmt3->execute();
        $count3 = $stmt3->rowCount();
        if ($count3 > 0) {
          while ($row3 = $stmt3->fetchAll(PDO::FETCH_ASSOC)) { // Loop for queries

            $warehouse_name = "<a href='../inventory/warehousedetails.php?uid=" . $row[0]['warehouse_id'] . "'>" . $row3[0]['name'] . "</a>";
          }
        }
      } else {
        $warehouse_name = "-";
      ...
      if (!empty($row[0]['withholding'])) {
        $withholding_perc = round(($row[0]['withholding'] / ($row[0]['subtotal'] - $row[0]['discount'])) * 100);
        $row[0]['withholding_perc'] = $withholding_perc . ".00";
      } else {
        $row[0]['withholding_perc'] = "0.00";
      }
      $row[0]['supplier_name'] = $supplier_name;
      $row[0]['withholding'] = round($row[0]['withholding'], 2);
      $row[0]['warehouse_name'] = $warehouse_name;
