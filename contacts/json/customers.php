...
if ($count > 0) {
  ...
  while ($row = $stmt->fetchAll(PDO::FETCH_ASSOC)) { // Loop for queries
    ...
    foreach ($row as $supp) {
      ...
      $supp_id = $supp['id'];
      $due_total = 0;
      $stmt = $con->prepare("SELECT SUM(`credit`) as `total_credit` FROM `sell_transactions` WHERE `supplier_id` = '$supp_id' AND debit = '0'");
      $stmt->execute();
      $row = $stmt->fetch(PDO::FETCH_ASSOC);
      $total_credit = $row['total_credit'];

      $stmt2 = $con->prepare("SELECT SUM(`debit`) as `total_debit` FROM `sell_transactions` WHERE `supplier_id` = '$supp_id' AND credit = '0'");
      $stmt2->execute();
      ...
      if ($supp['status'] == 1) {
        ...
        $status = '<span class="paid">' . $status_text . '</span>';
        ...
        $status = '<span class="closed">' . $status_text . '</span>';
        $check = '';
      }
      ...
      $supplier = array(
        ...
        'action' => '<a href="customerdetails.php?uid=' . $supp_id . '"><i class="fa-solid fa-eye"></i></a>',
      );
