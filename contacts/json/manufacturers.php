...
if ($count > 0) {
  ...
  while ($row = $stmt->fetchAll(PDO::FETCH_ASSOC)) { // Loop for queries
    ...
    foreach ($row as $supp) {
      ...
      $supp_id = $supp['id'];
      $due_total = 0;
      $stmt = $con->prepare("SELECT SUM(`credit`) as `total_credit` FROM `manufacturer_transactions` WHERE `supplier_id` = '$supp_id' AND debit = '0'");
      $stmt->execute();
      $row = $stmt->fetch(PDO::FETCH_ASSOC);
      $total_credit = $row['total_credit'];

      $stmt2 = $con->prepare("SELECT SUM(`debit`) as `total_debit` FROM `manufacturer_transactions` WHERE `supplier_id` = '$supp_id' AND credit = '0'");
      $stmt2->execute();
      ...
      $supplier = array(
        ...
        'action' => '<a href="manufacturerdetails.php?uid=' . $supp_id . '"><i class="fa-solid fa-eye"></i></a>',
      );
