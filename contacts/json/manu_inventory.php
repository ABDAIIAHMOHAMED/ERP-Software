...
if (isset($_GET['uid']) && !empty($_GET['uid'])) {
  ...
  $id = $_GET['uid'];
  $purchases = array();
  $stmt2 = $con->prepare("SELECT * FROM `av_items` WHERE `supplier_id` = '$id' AND `warehouse_id` != '314'");
  $stmt2->execute();
    ...
    while ($row2 = $stmt2->fetchAll(PDO::FETCH_ASSOC)) { // Loop for queries
      foreach ($row2 as $tra) {
        ...
        $purchase = array(
          ...
          'action' => '<a href="../inventory/itemdetails.php?uid=' . $tra["sku"] . '" target="_blank"><i class="fa-solid fa-eye"></i></a>',
        );
