...
if (isset($_GET['start_date']) && isset($_GET['end_date']) && !empty($_GET['start_date']) && !empty($_GET['end_date'])) {
  ...
  $stmt2 = $con->prepare("SELECT * FROM `manufacturer_orders` WHERE `quantity` > '0' AND `created_time` BETWEEN '$start_date' AND '$end_date' ");
  $stmt2->execute();
  ...
  if ($count2 > 0) {
    while ($row2 = $stmt2->fetchAll(PDO::FETCH_ASSOC)) { // Loop for queries
      foreach ($row2 as $tra) {
        ...
        $amount =    number_format((($tra['order_total'] - $tra['discount']) / $tra['quantity']), 2, '.', ',');
        $total = number_format($tra['order_total'], 2, '.', ',');
