...
if (isset($_POST["item_search"])) {
  if (!empty($_POST['item_search'])) {
    ...
    $stmt = $con->prepare("SELECT item_id, MAX(name) AS name, SUM(quantity) AS quantity, MAX(unit) AS unit, MAX(img) AS img, MAX(id) AS id FROM av_items WHERE name LIKE '%$item_search%' AND supplier_id NOT LIKE '%M%' GROUP BY item_id");
    $stmt->execute();
    $count = $stmt->rowCount();
    if ($count > 0) {
      while ($row = $stmt->fetchAll(PDO::FETCH_ASSOC)) { // Loop for queries
        foreach ($row as $item) {
          ...
          $stmt2 = $con->prepare("SELECT SUM(`quantity`) AS `total_quantity` FROM `av_items` WHERE `item_id` = '$item_id' AND `warehouse_id` != '314'");
          $stmt2->execute();
