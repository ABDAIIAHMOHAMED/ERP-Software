...
if (isset($_POST["item_search"])) {
  if (!empty($_POST['item_search'])) {
    ...
    $stmt = $con->prepare("SELECT * FROM `final_items` WHERE `name` LIKE '%$item_search%'  ");
    $stmt->execute();
    $count = $stmt->rowCount();
    if ($count > 0) {
      while ($row = $stmt->fetchAll(PDO::FETCH_ASSOC)) { // Loop for queries
        ...
        foreach ($row as $item) {
          echo '<li class="sug" data-id="' . $item['id'] . '" data-name="' . $item['name'] . '"><div class="img_cont_item"><img src="../assets/items/' . $item['img'] . '"></div><span>' . $item['name'] . '<span class="itemD">' . $item['code'] . '</span></span></li>';
        }
      }
    } else {
      echo '<li class="sug new_item_btn"><i class="fa-solid fa-plus"></i> Add New</li>';
