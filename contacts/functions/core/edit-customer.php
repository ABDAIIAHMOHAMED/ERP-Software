...
if (isset($_POST['edit']) && !empty($_POST['edit'])) {
    ...
    if (isset($_POST['fname'])) {
        if (!empty($_POST['fname'])) {
            ...
            if (preg_match('~[0-9]+~', $fname) || preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', $fname)) {
                ...
                if (strlen($fname) < 2) {
                    $errors["fname"] = "First name is too small minimun 2 character";
                } elseif (strlen($fname) > 128) {
                    $errors["fname"] = "First name is too long maximun 128 character";
                }
            }
        } else {
            if ($type == "1") {
    ...
    if (isset($_POST['mname'])) {
        if (!empty($_POST['mname'])) {
            ...
            if (preg_match('~[0-9]+~', $mname) || preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', $mname)) {
                ...
                } elseif (strlen($mname) > 128) {
                    $errors["mname"] = "Middle name is too long maximun 128 character";
                }
            }
        }
    }
    ...
    if (isset($_POST['lname'])) {
        if (!empty($_POST['lname'])) {
            ...
            if (preg_match('~[0-9]+~', $lname) || preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', $lname)) {
                ...
                } elseif (strlen($lname) > 128) {
                    $errors["lname"] = "Last name is too long maximun 128 character";
                }
            }
        }
    }
    ...
    if (empty(array_filter($errors))) {
        $stmt5 = $con->prepare("UPDATE `customers` SET  `taxid` = '$taxid', `status` =  '$status' ,`first_name` = '$fname'  , `middle_name` = '$mname', `last_name` = '$lname', `company_name` = '$cname' ,`email` = '$email', `phone` = '$phone' , `mobile` = '$mobile' , `fax` = '$fax' , `website` = '$website' , `other` = '$other' , `notes` = '$notes' , `street` = '$street' , `city` = '$city' , `state` = '$state' , `country` = '$country' , `acc_num` = '$acc_num' , `last_modification` = '$lmodification'  WHERE `id` = '$id'");
