    ...
    <div class="att-part">
      <div class="title-head">
        <div class="headso">
          ...
          <hr class="head-line">
          <p class="navigator"><a href="<?= $path ?>"><?= $lang['dashboard'] ?></a> / <?= $lang['customers'] ?></p>
        </div>
          ...
          <button class="export"><i class="fa-solid fa-download"></i>
            <ul class="export-options">
              <li class="option" id="ExportReporttoPdf">
                <div><i class=" fa-solid fa-print"></i> <?= $lang['print'] ?></div>
              </li>
              <li class="option btn-print" id="ExportReporttoExcel">
                <div><i class="fa-solid fa-file-csv"></i> <?= $lang['excel'] ?></div>
              </li>
            </ul>
          </button>
          <?php if (check_user_role(1) == 1 || check_user_role(100) == 1 || check_user_role(102) == 1) {
          ?>
            <button class="new-btn"><?= $lang['new customer'] ?></button>
          <?php
          }
          ?>
        </div>
      ...
      <div class="request-section filter-section">
        <form action="" method="POST">
          ...
          <h6 class="fhead"><?= $lang['filteration'] ?></h6>
          <div class="inputs inputs-grid">
            ...
            <div class="inputs">
              <label for="type_filter"><?= $lang['type'] ?></label>
            ...
            <div class="inputs">
              <label for="create-date-filter"><?= $lang['created date'] ?></label>
              <input type="date" name="date" id="create-date-filter">
            </div>
            <div class="inputs">
              <label for="status_filter"><?= $lang['status'] ?></label>
              <select name="type" id="status_filter">
                <option value="active"><?= $lang['active only'] ?></option>
                <option value="inactive"><?= $lang['inactive only'] ?></option>
                <option value="all" selected><?= $lang['inactive included'] ?></option>
              </select>
            </div>
          </div>
        </form>
      ...
      <div class="employee-section request-section">
        <form action="" method="POST" class="list-form">
          <div class="main-table-style">
            <div class="serch">
              <input type="text" id="name-search" value="" placeholder="<?= $lang['search'] ?>">
            </div>
            <table id="supplierslist" style="width:100% !important;">
              <thead>
                <tr>
                  ...
                  <th><?= $lang['phone'] ?></th>
                  <th><?= $lang['email'] ?></th>
                  <th><?= $lang['Due Balance'] ?> (<?= $lang['currency'] ?>)</th>
                  <th><?= $lang['status'] ?></th>
          ...
          <input type="submit" class="delete-btn" name="inactive-submit" value="<?= $lang['Mark as Inactive'] ?>">
        </form>
      </div>
      ...
      ?>
        <div class="add_new">
          <div class="custome-head">
            <h3><?= $lang['new customer'] ?></h3>
            <button class="close"><i class="fa-solid fa-xmark"></i></button>
          </div>
          <form class="" id="new_supplier" action="" method="POST">
            <div class="custom-form">
              <div class="sectra">
                <span class="sm-head"><?= $lang['Contact info'] ?></span>
                <span class="ask">
                  <label for="ask"><?= $lang['display as a company'] ?></label>
                  <input type="checkbox" name="type" value="1" id="ask">
                </span>
                <span class="ask">
                  <label for="ask"><?= $lang['active'] . " " . $lang['?'] ?> </label>
                ...
                <div class="form-section">
                  ...
                  <div class="inputs gd2">
                    <label for="mname"><?= $lang['Middle Name'] ?></label>
              ...
              <div class="sectra">
                <span class="sm-head"><?= $lang['Address'] ?></span>
                <div class="form-section">
                  <div class="inputs gd6">
                  ...
                  <div class="inputs gd3">
                    <label for="city"><?= $lang['City/Town'] ?></label>
                  ...
                  <div class="inputs gd3">
                    <label for="postal"><?= $lang['Postal Code'] ?></label>
                ...
                <span class="sm-head"><?= $lang['Payement and Billing'] ?></span>
                <div class="form-section">
                  <div class="inputs gd6">
                    <label for="acc_num"><?= $lang['Account no.'] ?></label>
                    <input type="text" id="acc_num" value="" class="" name="acc_num">
                    <span class="error-field"></span>
                  </div>
                  <div class="inputs gd6">
                    <label for="taxid"><?= $lang['Tax Reg ID.'] ?></label>
                    <input type="text" id="taxid" value="" class="" name="tax_id">
                    <span class="error-field"></span>
                  </div>
                </div>
              </div>
            ...
            <input type="submit" class="add" value="<?= $lang['save'] ?>">
          </form>
        </div>
      <?php
      }
    ...
    <script>
      $(document).ready(function() {
        ...
        $("#ExportReporttoPdf").on("click", function() {
          ...
          <?php if ($curr_lang == 'ar') {
          ?>
            printWindow.document.write('<style>body{direction:rtl;}table th ,table td{text-align: right;}</style>');
          <?php
          }
          ?>
          printWindow.document.write('</head><body><div class="container"><div class="header_container"><div class="lt-head"><h3><?= $lang["customers"] ?></h3><span class="dt-range"></span></div><div class="rt-head"><div class="logo_img"><img src="../images/logo.png" alt="Company_logo" /></div></div></div><table>');
          printWindow.document.write(barcodeDiv.innerHTML);
          printWindow.document.write('</table><div class="sub_header"><span><?= $lang['report at'] ?> ' + currentTime + '</span><span><?= $lang["By"] ?> ' + userDate + '</span></div><div class="copyright-section">© 2023 <?= $lang['right_rec'] ?></div></div></body></html>');
        });
        ...
        $('#new_supplier').submit(function(event) {
          ...
          $.ajax({
            type: 'POST',
            url: 'functions/core/add-customer.php',
            data: $(this).serialize(),
