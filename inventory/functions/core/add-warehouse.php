...
if (isset($_POST['add']) && !empty($_POST['add'])) {
  ...
  if (empty(array_filter($errors))) {
    $stmt4 = $con->prepare("INSERT INTO `warehouses`(`code`,`name`,`phone`,`notes`,`street`,`city`,`state`,`postal`,`country`,`created_time`,`status`) VALUES ('$code','$name','$phone','$notes','$street','$city','$state','$postal','$country','$cdate','$status')");
    $stmt4->execute();
