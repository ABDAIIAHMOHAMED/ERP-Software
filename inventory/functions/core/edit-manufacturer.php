...
if (isset($_POST['edit']) && !empty($_POST['edit'])) {
    ...
    if (empty(array_filter($errors))) {
        $stmt5 = $con->prepare("UPDATE `manufacturers` SET  `status` =  '$status' ,`first_name` = '$fname'  , `middle_name` = '$mname', `last_name` = '$lname', `company_name` = '$cname' ,`email` = '$email', `phone` = '$phone' , `mobile` = '$mobile' , `fax` = '$fax' , `website` = '$website' , `other` = '$other' , `notes` = '$notes' , `street` = '$street' , `city` = '$city' , `state` = '$state' , `country` = '$country' , `acc_num` = '$acc_num' , `last_modification` = '$lmodification'  WHERE `id` = '$id'");
