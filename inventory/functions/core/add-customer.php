...
if (isset($_POST['add']) && !empty($_POST['add'])) {
    ...
    if (isset($_POST['as_of'])) {
        if (!empty($_POST['as_of'])) {
            $as_of = filter_var($_POST['as_of'], FILTER_SANITIZE_STRING);
        } else {
            $errors["as_of"] = "This field is required and cannot be empty";
        }
    }
    ...
    if (isset($_POST['payment_method'])) {
        if (!empty($_POST['payment_method'])) {
            ...
            if (!in_array($payment_method, $valid_methods)) {
                $errors["payment_method"] = "Payment Method is not available right now !";
            }
        } else {
            $errors["payment_method"] = "This field is required and cannot be empty";
        }
    }

    if (isset($_POST['open_balance'])) {
        if (!empty($_POST['open_balance'])) {
            $open_balance = filter_var($_POST['open_balance'], FILTER_SANITIZE_STRING);
            if (is_numeric($open_balance)) {
                if (preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', $open_balance)) {
                    $errors["open_balance"] = "Opening Balance can't contain special character";
                } else {
                    if ($open_balance == trim($open_balance) && strpos($open_balance, ' ') !== false) {
                        $errors["open_balance"] = "Opening Balance can't contain any spaces";
                    }
                }
            } else {
                $errors["open_balance"] = "Opening Balance can't contain any characters";
            }
        }
    }
    if (empty(array_filter($errors))) {
        $stmt4 = $con -> prepare("INSERT INTO `customers`(`first_name`,`middle_name`,`last_name`,`company_name`,`is_company`,`phone`,`mobile`,`fax`,`email`,`website`,`other`,`notes`,`street`,`city`,`state`,`postal`,`country`,`tax_id`,`payment_method`,`open_balance`,`as_of`,`created_time`,`last_modification`) VALUES ('$fname','$mname','$lname','$cname','$type','$phone','$mobile','$fax','$email','$website','$other','$notes','$street','$city','$state','$postal','$country','$tax_id','$payment_method','$open_balance','$as_of','$cdate','$lmodification')");
        $stmt4 -> execute();
        get_customers();
