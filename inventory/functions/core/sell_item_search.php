...
if (isset($_POST["item_search"])) {
  if (!empty($_POST['item_search'])) {
    ...
    $stmt = $con->prepare("SELECT DISTINCT `item_id`,`name`,`img` FROM `av_items` WHERE `name` LIKE '%$item_search%' AND `supplier_id` LIKE '%M%' ");
    $stmt->execute();
    $count = $stmt->rowCount();
    if ($count > 0) {
      while ($row = $stmt->fetchAll(PDO::FETCH_ASSOC)) { // Loop for queries
        foreach ($row as $item) {
          echo '<li class="sug" data-id="' . $item['item_id'] . '" data-name="' . $item['name'] . '"><div class="img_cont_item"><img src="../assets/items/' . $item['img'] . '"></div><span>' . $item['name'] . '<span class="itemD">' . $item['item_id'] . '</span></span></li>';
        }
      }
    } else {
